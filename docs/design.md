...
- 新增：主流程封装为 `Invoke-RootOrchestration`，显式管理 Transcript 生命周期并兜底捕获未处理异常。

## 核心流程
```
Autounattend -> root.ps1
  -> Invoke-RootOrchestration
     -> Start-Transcript (标记已启动)
     -> 读取 RootPhase
     ...
     -> Stop-Transcript (仅在启动成功时)
```

## 关键设计点
- **阶段控制**：通过 `RootPhase` 注册表值与 `RunOnce` 实现二阶段执行；保持幂等。
- **步骤包装**：`Invoke-Step` 统一捕获并记录每个子步骤的异常，避免全局中断。
- **日志可靠性**：`transcriptStarted` 标志确保只在成功启动 Transcript 后才尝试停止，避免 Stop-Transcript 抛错导致误判。
...
## 设计取舍
- 保留嵌套 `try/catch`：每个外部依赖调用（安装器、注册表写入）均局部防护，确保单点失败不终止流程。
